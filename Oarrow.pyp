...
from c4d import BaseObject, BaseTag, GeListNode
...
class Oarrow(c4d.plugins.ObjectData):
    # This holds the possible flat orientations for each orientation that the arrow can have.
    flat_orientations_per_orientation = None
    # Couldn't figure out how to access the already existing axis names from the C4D module
    # so they're here now.
    axis_names = None
    ...
    def Init(self, op, isCloneInit):
        ...
        self.InitAttr(op, c4d.BaseList2D, c4d.OARROW_TARGET)
        self.InitAttr(op, float, c4d.OARROW_TARGET_OFFSET)
        ...
        op[c4d.OARROW_TARGET] = None
        op[c4d.OARROW_TARGET_OFFSET] = 0.0
    ...
    def __setAxis(self, op: c4d.BaseObject, points: list[c4d.Vector]):
        """ Rotates the geometry based on the selected orientation. """
        orientation = op[c4d.PRIM_AXIS]

        # These permutations assume that the object was created with Z+ as default orientation.
    ...
    def Message(self, node: GeListNode, type: int, data: object) -> bool:
        if type == c4d.MSG_MENUPREPARE:
            return self.__on_MSG_MENUPREPARE(node)
    ...
    def __on_MSG_MENUPREPARE(self, node: GeListNode) -> bool:
        """ Prepare some things for the object. """
        node.SetPhong(True, True, c4d.utils.DegToRad(40.0))

        if not isinstance(node, c4d.BaseObject):
            return False
        
        self.__create_length_calculator_tag(node)
        self.__create_constraint_tag(node)
        
        return True

    def __create_length_calculator_tag(self, node: c4d.GeListNode):
    ...
    def __create_constraint_tag(self, node: c4d.GeListNode):
        ...
        if aim_constraint_tag is None:
            ...
            # ToDo: Uncomment once release is ready.
    ...
    def __on_MSG_DESCRIPTION_POSTSETPARAMETER(self, node: GeListNode, data: object) -> bool:
        """ React to various value changes. """
        ...
        if target_changed:
            self.__on_target_changed(node, aim_constraint_tag)

        elif orientation_changed:
            if not self.__on_orientation_changed(node, aim_constraint_tag):
                return False
    ...
    def __on_target_changed(self, node: c4d.GeListNode, aim_constraint_tag: c4d.BaseTag):
        target = node[c4d.OARROW_TARGET]
    ...
    def __on_orientation_changed(self, node: c4d.GeListNode, aim_constraint_tag: c4d.BaseTag):
        orientation = node[c4d.PRIM_AXIS]
        
        if orientation is None:
            return False

        self.__determine_aim_constraint_axis(node, orientation, aim_constraint_tag)
        self.__determine_flat_orientation(node, orientation)

        return True

    def __determine_aim_constraint_axis(self, node: c4d.GeListNode, orientation: int, aim_constraint_tag: c4d.BaseTag):
    ...
    def __determine_flat_orientation(self, node: c4d.GeListNode, orientation: int):
    ...
    def GetDDescription(self, node: c4d.GeListNode, description: c4d.Description, flags: int) -> bool | Tuple[bool, int]:
        ...
        if single_id is None or flat_orientation_id.IsPartOf(single_id)[0]:
            if self.__prepare_flat_orientation_description(node, description) == False:
                return False

        return (True, flags | c4d.DESCFLAGS_DESC_LOADED)
    
    def __prepare_flat_orientation_description(self, node: c4d.GeListNode, description: c4d.Description):
        orientation = node[c4d.PRIM_AXIS]
    ...
    def GetDEnabling(self, node: c4d.GeListNode, id: c4d.DescID, t_data: object, flags: int, itemdesc: c4d.BaseContainer) -> bool:
        ...
        if rotation_segments_id.IsPartOf(id)[0]:
            flat_enable = node[c4d.OARROW_FLAT_ENABLE]
            return False if flat_enable == 1 else True
        
        elif flat_direction_id.IsPartOf(id)[0]:
            flat_enable = node[c4d.OARROW_FLAT_ENABLE]
            return True if flat_enable == 1 else False
        
        elif length_id.IsPartOf(id)[0]:
            link_is_set = node[c4d.OARROW_TARGET] is not None
            return not link_is_set
        
        elif target_offset_id.IsPartOf(id)[0]:
            target_is_set = node[c4d.OARROW_TARGET] is not None
...
class Tcalculatearrowlength(c4d.plugins.TagData):
    def Execute(self, tag: BaseTag, doc: BaseDocument, op: BaseObject, bt: BaseThread, priority: int, flags: int) -> int:
        ...
        target = op[c4d.OARROW_TARGET]

        # If no target is set the calculation can be skipped.
        if target is None:
        ...
        offset = op[c4d.OARROW_TARGET_OFFSET]

        if offset is None:
            offset = 0.0

        calculated_length = distance_to_target - offset

        op[c4d.OARROW_LENGTH] = calculated_length
